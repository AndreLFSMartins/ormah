...
Always `AndreLFSMartins/ormah` — the `fork` remote. **Never** `r-spade/ormah`, which is where
both `origin` and `upstream` point (see `FORK-WORKFLOW.md`; that layout is deliberate). Do NOT
infer the repo from `git remote -v` here — the obvious inference is the wrong one.

`gh repo set-default` is already set to `AndreLFSMartins/ormah`, so a bare `gh issue ...` resolves
correctly inside this clone. If a command ever prompts for the repo, or you are outside the clone,
