...
- **`content` is nullable in `nodes`** (`src/ormah/index/schema.sql`, verified against the live DB). `duplicate_merger.py:134` does `node_row["content"][:2000]` with no guard. Today 0 of 4031 rows are NULL, so this is armed, not firing — the detector script in Task 3 and Task 5 must skip NULL-content rows rather than crash mid-run.
