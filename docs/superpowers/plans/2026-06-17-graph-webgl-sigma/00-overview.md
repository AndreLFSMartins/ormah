...
| `ui/src/graph/graphModel.ts` | pure: `buildGraph` (payload → graphology) + `applyAppearance` (in-place restyle, M4) | 3 |
| `ui/src/graph/graphModel.test.ts` | unit tests (happy, missing space/tier, empty, isolated, applyAppearance) | 3 |
| `ui/src/graph/sigmaReducers.ts` | pure: `makeNodeReducer`/`makeEdgeReducer` (hover, multi-highlight, 4-dim legend dim) | 4 |
| `ui/src/graph/sigmaReducers.test.ts` | unit tests (hover, highlightSet, space/tier/role/edge dim) | 4 |
| `ui/src/graph/forceLayout.ts` | injectable FA2 worker wrapper: start/stop/kill/reheat + settle + static fallback | 5 |
| `ui/src/graph/forceLayout.test.ts` | unit tests via injected fake worker (settle, reheat, fallback) | 5 |
| `ui/src/components/GraphView.tsx` | orchestrator: mount sigma, run layout, hover/drag/tooltip, ref API+fit, focusNodeId, zoom, full legend | 6 |
| `ui/playwright/test_graph_layout.py` | visual test: settle, canvas present, **organic non-grid** (NN-distance variance) | 7 |
| `ui/playwright/test_graph_drag.py` | drag test: node moves + post-release reheat | 7 |
...
4. `04-sigma-reducers.md` — hover / multi-highlight / 4-dimension legend-dim reducer factories, with tests.
5. `05-force-layout.md` — injectable FA2 worker wrapper (settle/reheat/static fallback), with tests.
6. `06-graphview-rewrite.md` — sigma orchestrator: mount, hover/drag/tooltip, ref API + highlight-fit, focusNodeId, zoom, full legend, in-place restyle (the big one).
7. `07-playwright-visual.md` — Python Playwright: organic non-grid assertion + node-drag test.
8. `08-cleanup-and-verify.md` — remove cytoscape, lint/build/test green.

> **Council review (2026-06-17, R1 cursor+codex):** plan adjusted before implementation — H1 (restore highlight viewport fit), H2 (test asserts organic non-grid by topology, NOT per-space separation — option A has no space-cluster criterion), M1 `focusNodeId`, M2 full legend (tier/role/edge), M3 zoom controls, M4 in-place restyle (no remount), M5 injectable worker + static fallback + tests, L1 DEV-only debug handle. See `.council/council-result.md`.
