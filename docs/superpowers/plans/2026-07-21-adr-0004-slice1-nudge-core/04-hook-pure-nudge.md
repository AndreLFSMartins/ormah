# Task 4: Hook becomes a pure nudge — client cursor machinery deleted
...
**Interfaces:**
- Consumes: `POST /ingest/nudge` from Task 3.
...
5. **Lock a separate, stable file** (council R9 — now **measured**, not theoretical).
   `flock` is an INODE lock: if the drain rewrites the outbox via `os.replace`, an appender
   holding the OLD inode still believes it owns the lock, and mutual exclusion silently
   evaporates. Prototype, three appenders + one rotating drain over 2 s: locking the outbox
   inode produced **1140 mutual-exclusion violations**; a dedicated
   `whisper-nudge-outbox.lock` produced **0**.
   ⚠️ The trap only fires when the lock fd is **held across** the replace — the cheap
   variant (open by path, lock, append, unlock, close, every time) is immune. Do not let a
   reviewer's inability to reproduce it become an argument for dropping the rule: the
   correct fix costs one file.
