...
/ delete / soft_delete / list_deleted) except the one exception the ticket
declares: whether an absent id was left out of `_id_cache` is not externally
observable, so that one case pokes `store._id_cache` directly — same pattern
as test_file_cache.py::test_stale_cache_entry_recovers.
"""
...
def test_deleting_one_of_a_colliding_pair_does_not_resurrect_it_as_the_other(file_store):
    ...
    entries = file_store.list_deleted()
    assert len(entries) == 1
    tombstone_id, deleted_at, _path = entries[0]
    assert tombstone_id == first.id
