...
def test_a_naive_last_accessed_on_one_row_does_not_abort_the_whole_run(engine):
    """A single bad timestamp must skip that node, not the entire decay pass.

    `now - anchor` sits outside the inner try/except in the code under test, so a
    naive `last_accessed` (no tzinfo — e.g. hand-edited or externally generated
    frontmatter) raises `TypeError` when subtracted from the aware `now`. That
    error is caught only by the outer `except Exception` wrapping the whole loop,
    which silently disables decay for every node in the store, not just the bad
    one. This test seeds one poisoned node alongside two well-formed, decay-
    eligible ones and asserts the well-formed ones are still demoted.
    """
    poisoned_id, _ = engine.remember(CreateNodeRequest(
        content="Node with a naive last_accessed",
        type=NodeType.fact,
        tier=Tier.working,
        title="Poisoned",
    ))
    good_id_1, _ = engine.remember(CreateNodeRequest(
        content="Well-formed stale node one",
        type=NodeType.fact,
        tier=Tier.working,
        title="Good one",
    ))
    good_id_2, _ = engine.remember(CreateNodeRequest(
        content="Well-formed stale node two",
        type=NodeType.fact,
        tier=Tier.working,
        title="Good two",
    ))

    naive_str = (datetime.now() - timedelta(days=30)).isoformat()
    assert datetime.fromisoformat(naive_str).tzinfo is None, (
        "fixture no longer produces a naive timestamp; it would not reproduce the defect"
    )
    engine.db.conn.execute(
        "UPDATE nodes SET last_accessed = ?, last_review = NULL WHERE id = ?",
        (naive_str, poisoned_id),
    )
    engine.db.conn.commit()

    _make_stale(engine, good_id_1)
    _make_stale(engine, good_id_2)
    _make_decayable(engine, poisoned_id)
    _make_decayable(engine, good_id_1)
    _make_decayable(engine, good_id_2)

    run_decay(engine)

    assert _get_tier(engine, good_id_1) == "archival"
