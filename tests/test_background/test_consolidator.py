...
class TestRunConsolidationSplits:
    ...
    def test_oversized_cluster_becomes_two_consolidations(self, monkeypatch, engine):
        ...
        seen = []
        monkeypatch.setattr(
            consolidator, "_consolidate_cluster", lambda eng, sub, epoch=None: seen.append(sub)
        )
    ...
    def test_exhausted_budget_warns_once_not_once_per_cluster(self, monkeypatch, engine, caplog):
        ...
        called = []
        monkeypatch.setattr(
            consolidator, "_consolidate_cluster", lambda eng, sub, epoch=None: called.append(sub)
        )
