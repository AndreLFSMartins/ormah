...
def test_enforce_setting_drops_material_without_a_code_guard(engine, monkeypatch, tmp_path):
    """Lean path (ADR-0002, 2026-07-20): there is no _RELEVANCE_ENFORCE_AVAILABLE code
    guard anymore. The runtime setting alone flips enforcement — set it and Material is
    actually dropped (with a best-effort ledger record), flipped only after a shadow run
    is reviewed on real data."""
    from ormah.engine import relevance_quarantine as q
    monkeypatch.setattr(q, "quarantine_path", lambda s: tmp_path / "q.jsonl")
    engine.settings.ingest_relevance_gate = True
    engine.settings.ingest_relevance_gate_enforce = True
