...
def _prompt_exceeds_provider_capacity(settings, prompt: str) -> int | None:
    """The prompt's ESTIMATED token count when it overflows the usable input window, else None.

    (None also covers "unknown": a provider whose window we cannot introspect.)
...
class MemoryEngine:
    ...
    def update_node(self, node_id: str, req: UpdateNodeRequest) -> str | None:
        ...
        # regardless of any space/space_locked the request tried to set. Runs before the
        # no-op guard so a reverted space edit is correctly seen as "nothing changed".
        apply_identity_space_invariants(node)
    ...
    def _maybe_get_onboarding_nudge(self, space: str | None = None) -> str:
        ...
        onboarding_nudge = (
            "## Ormah: First-time setup\n\n"
            "This is the user's first Ormah session. No profile memories exist yet.\n\n"
            "STOP — do not start any work on the user's request yet. "
            "Your entire first response must be the opt-in message below and nothing else.\n\n"
            "Say this naturally, keeping it warm and brief:\n\n"
            ...
            "1. Name + optional onboarding material (already asked above). If they share a CV, GitHub "
            "profile, personal site, short bio, LinkedIn page, portfolio, resume, or other profile "
            "source, fetch/read it if accessible and mine it thoroughly. Extract all durable profile "
            "context worth remembering, not just a short summary. Store multiple atomic memories for "
            "identity, current role, organizations, career history, skills, education, public links, "
            "domains of expertise, notable projects, and explicit current focus areas or goals when "
            "the source contains them. Store 5-15 self-contained memories if the source has enough "
            "signal. Use 'profile' only as a tag, never as the memory type. Choose valid memory types "
            "such as person, fact, event, project, goal, preference, or observation. "
            "If the page is blocked or low-confidence, do not infer details; ask for a CV, "
            "GitHub profile, personal site, or short bio instead.\n"
            ...
            "and immediately proceed to their task. Store "
            "remember(content='User skipped onboarding', type='preference', about_self=True, "
            "space=None) "
            "so this nudge does not appear again.\n\n"
            "Store every onboarding answer with remember(..., about_self=True, space=None). "
        ...
        for mem in extracted:
            ...
            if self.settings.ingest_relevance_gate:
                if prov not in ("material", "product"):
                    missing_label += 1
                if prov == "material":
                    # SHADOW (default) records the would-drop but keeps the memory;
                    # ENFORCE (ORMAH_INGEST_RELEVANCE_GATE_ENFORCE=true) actually drops.
                    # No code-disable: the runtime flag is the switch, flipped only after
                    # a shadow run's would-drops are reviewed on real data (ADR-0002).
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                if overflow is not None:
                    from ormah.ingest_capacity import usable_input_tokens
                    ...
                    logger.error(
                        ...
                        overflow, usable_input_tokens(self.settings),
                    )
