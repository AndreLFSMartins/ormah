...
@restore_aware_job
def run_consolidation(engine, epoch: int) -> dict | None:
    ...
    for sub in capped:
        try:
            _consolidate_cluster(engine, sub, epoch)
...
def _consolidate_cluster(engine, cluster: list[dict], epoch: int | None = None) -> None:
    ...
    if not summary:
        _record_signature(engine, sig, epoch)
        return

    node_ids = [n["id"] for n in cluster]
    new_id = _apply_consolidation(engine, node_ids, title, summary, node_type, epoch=epoch)
...
def _record_signature(engine, sig: str, epoch: int | None = None) -> None:
    # Same reasoning as auto_linker's watermark: marking the cluster checked after a restore
    # rolled the consolidation back would stop it from ever being reconsidered (#240).
    # No epoch (direct callers, e.g. tests) means no epoch guard, as elsewhere in this module.
    from contextlib import nullcontext
